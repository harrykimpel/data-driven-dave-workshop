...
class Screen(object):
    ...
    def moveScreenX(self, map, amount, tileset, ui_tileset, player, level_number):
        ...
        reached_level_right_boundary = (
            self.x_pos + self.getWidthInTiles() >= map.getWidth())
...
class Map(object):
    ...
    def buildMapMatrix(self):
        self.node_matrix = [[Tile() for i in range(self.width)]
    ...
    def checkPlayerCollision(self, player_x, player_y, player_width, player_height, solid_only=False):
        ...
        x_right = int((player_x + player_width-1 -
                      TOLERANCE_VALUE) // WIDTH_OF_MAP_NODE)
    ...
    def tileFromText(self, text_tile):
        ...
        if text_tile == "DO":
            ...
            if text_tile[1] == '0':
                ...
                type = "magic wand"
            return Item("items", gfx_id, scores[1], type)
        elif text_tile[0] == 'P':
...
class Tile(object):
    ...
    def getGraphic(self, tileset):
        ...
        size_of_rectangle = (tile_width * TILE_SCALE_FACTOR,
                             tile_height * TILE_SCALE_FACTOR)
        cropped_tile = pygame.transform.scale(
            subtileset_image.subsurface(rectangle), size_of_rectangle)
...
class AnimatedTile(Tile):
    ...
    def getGraphic(self, tileset):
        ...
        if (self.anim_timer == 0):
            ...
            number_of_tile_frames = int(
                tile_frames[0].get_rect().size[0]/tile_frames[2])
...
class Player(Dynamic):
    ...
    MAX_SPEED_X = 0.6
    MAX_SPEED_Y = 0.6
    # factor to be used when not falling (x speed only hits its maximum when falling)
    X_SPEED_FACTOR = 0.75
    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            self.gfx_id = 0
            self.cur_state = STATE.BLINK
            # The velocity and direction in the y axis (module + value)
            self.velocity_y = 0
            # The velocity in the x axis (only value)
            self.velocity_x = self.MAX_SPEED_X * self.X_SPEED_FACTOR
            # Shows the current direction of movement (-1 = left, 1 = right, 0 = none)
    ...
    def updatePosition(self, player_x, player_y, level, screen_max_height):
        ...
        walked_into_pit = (
            not level.isPlayerCollidingWithSolid(player_x, player_y + 1))
        if self.cur_state == STATE.WALK and walked_into_pit:
            self.setFallingState()

        # Move X: START
        # Tries to walk to the direction the player's going
        player_newx = player_x + self.velocity_x * self.direction_x.value
        solid_collision = level.isPlayerCollidingWithSolid(
            player_newx, player_y)

        # If a collision occurs,
        if solid_collision:
            # undo the movement
            player_newx = player_x
            # If player's falling and released movement keys,
            if self.cur_state == STATE.FALL:
                # stop the uncontrolled fall
                self.clearXMovement()
        # Move X: END
        ...
        # If the player moved, updates the animation
        if (player_x != player_newx or player_y != player_newy) and self.cur_state != STATE.ENDMAP:
            self.updateAnimator()
...
class PlayerAnimator(object):
    ...
    def update(self, current_state):
        ...
        # Only updates if it's time to update (the counter reached its maximum)
        if self.animation_counter < self.ANIMATION_COUNTER_MAX:
            self.animation_counter = self.animation_counter + 1
        else:
            self.animation_counter = 0
            # Rotates the number that indexes the list of frames of that STATE.
            if self.animation_index < len(self.animation_index_list[current_state]):
                # Basically means that the animation frames will alternate in the list's order.
...
class Enemy(Dynamic):
    ...
    MOVEMENT_TYPES = ("idle", "straight", "back_and_forth", "ellipse",
                      "cross", "spider", "special_purple", "special_red")
